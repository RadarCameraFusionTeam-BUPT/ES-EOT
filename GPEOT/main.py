...
R_0 = Rt.from_quat(q_0).as_matrix()

# Basis points
u_f = spherical_angles_3d(theta_points, phi_points)
N_f = len(u_f)
...
f = th.zeros(N_f, dtype=th.float64)
P_f = K_uf_uf.clone().detach()
...
for i in range(len(radar_point)):
    ...
    ## Sphere surface data
    # m_k = th.tensor(generate_sphere_measurements(10, 5.0, R_line), dtype = th.float64)
...
np.save(os.path.join(os.path.dirname(__file__),\
    'GPEOT_result.npy'), res, allow_pickle = True)